...
window.onload = () => {
  ...
  function playAnimation(anim) {
    anim.play()
    anim.onComplete = () => {
      anim.gotoAndPlay(0) // 再生完了後、最初のフレームに戻す
    }
  }
  ...
  let displayMessage = (message, senderId) => {
    console.log('displayMessage', message, senderId)
  ...
  document.getElementById('sendBtn').addEventListener('click', () => {
    ...
    if (inputText) {
      ...
      console.log('send message', { message: inputText, senderId: clientId })
  ...
  socket.on('message', (data) => {
    console.log(data.senderId, clientId)
